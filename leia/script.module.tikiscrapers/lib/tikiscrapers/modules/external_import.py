...
class ExternalImporter():
    ...
    def removeExternal(self):
        try:
            ...
            if len(selectList) == 0: return control.infoDialog('No External Scrapers to Remove', time=5000)
            select = dialog.select("Choose External Module", selectList, useDetails=True)
            control.idle()
            if select == -1: return control.openSettings('0.5')
