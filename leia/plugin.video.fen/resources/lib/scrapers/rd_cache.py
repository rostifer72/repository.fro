...
class RealDebridSource:
	...
	def results(self, info):
		try:
			...
			threads = []
			threads.append(Thread(target=self._scrape_cloud))
			threads.append(Thread(target=self._scrape_downloads))
	...
	def make_downloads_item(self, item):
		finished_item = {}
		finished_item['folder_name'] = item['filename']
		finished_item['url_link'] = item['download']
		finished_item['bytes'] = item['filesize']
		finished_item['path'] = item['filename']
