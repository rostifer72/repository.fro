...
def base32_to_hex(_hash):
	from base64 import base32decode
	try: _hex = base32decode(_hash).encode('hex')
	except: _hex = None
	return _hex
...
def make_day(today, date, date_format, use_words=True):
	...
	if use_words:
		if day_diff == -1:
			day = local_string(32848).upper()
		elif day_diff == 0:
			day = local_string(32849).upper()
		elif day_diff == 1:
			day = local_string(32850).upper()
		elif 1 < day_diff < 7:
			day = date.strftime('%A').upper()
			day = local_string(translate_day(day))
	return day
...
def logger(heading, function):
	...
	xbmc.log('###%s###: %s' % (heading, function), notice)
...
def title_key(title, ignore_articles=None):
	...
	try:
		...
		return title[offset:]
		sorted(self.list, key=lambda k: re.sub(r'(^the |^a |^an )', '', k['name'].lower()))
	except: return title
...
def sort_for_article(_list, _key, ignore_articles=None):
	if ignore_articles is None:
		from modules.settings import ignore_articles as ignore
		ignore_articles = ignore()
	if not ignore_articles: return sorted(_list, key=lambda k: k[_key])
	import re
	return sorted(_list, key=lambda k: re.sub(r'(^the |^a |^an )', '', k[_key].lower()))
