...
from modules.nav_utils import build_url, add_dir, setView, notification, translate_path, execute_builtin, show_text
...
class People():
	...
	def main(self):
		...
		menus = [(ls(32028), 'people_search.media_results', True, 'movies'), 
				(ls(32029), 'people_search.media_results', True, 'tvshows'),
				('%s %s' % (ls(32028), ls(32796)), 'people_search.media_results', True,  'movies_directed'),
				(ls(32797), 'people_search.biography_results', False),
				(ls(32798), 'people_search.image_results', False),
				('%s %s' % (ls(33045), ls(32798)), 'people_search.tagged_image_results', False),
				('%ss' % ls(32491), 'people_search.video_results', True)]
		if settings.addon_installed('script.extendedinfo'):
			menus.append((ls(32610), 'people_search.extended_info', False))
		for menu_item in menus:
			try: params['media_type'] = menu_item[3]
			except: pass
			self._add_dir(menu_item[0], menu_item[1], params, isFolder=menu_item[2])
	...
	def extended_info(self):
		execute_builtin('RunScript(script.extendedinfo,info=extendedactorinfo,id=%s)' % self.actor_id)
	...
	def biography_results(self):
		...
		return show_text(bio_info.get('name'), text=text)

	def image_results(self):
		from indexers.images import Images
		params = {'mode': 'people_image_results', 'actor_name': self.actor_name, 'actor_id': self.actor_id, 'actor_image': self.actor_image, 'page_no': self.page_no, 'rolling_count': self.count}
		Images().run(params)

	def tagged_image_results(self):
		from indexers.images import Images
		params = {'mode': 'people_tagged_image_results', 'actor_name': self.actor_name, 'actor_id': self.actor_id, 'actor_image': self.actor_image, 'page_no': self.page_no, 'rolling_count': self.count}
		Images().run(params)
	...
	def media_results(self, media_type, page_no, letter):
		...
		def _add_misc_dir(url_params, list_name=ls(32799), info=ls(32800), iconImage='item_next.png'):
			...
			listitem.setArt({'icon': os.path.join(icon_directory, iconImage), 'fanart': fanart})
		...
		try:
			...
			data = sort_for_article(result, key, settings.ignore_articles())
