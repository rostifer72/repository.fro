...
from modules.nav_utils import build_url, setView, remove_meta_keys, notification, translate_path, sleep, execute_builtin, external_browse
...
from metadata import tvshow_meta, season_episodes_meta, retrieve_user_info, check_meta_database
	...
	def _process_eps(item):
		try:
			...
			episode_item = {"season": season, "episode": episode, "meta": meta, "curr_last_played_parsed": datetime_object, "action": "next_episode",
							...
							"unaired_color": unaired_color, "watched_indicators": watched_indicators}
...
def build_next_episode_manager(params):
	...
	check_meta_database()
	__handle__ = int(argv[1])
	NEXT_EP_UNWATCHED = get_setting('nextep.unwatched_colour', 'red')
	threads = []
	sorted_list = []
...
def nextep_execute(nextep_info):
	execute_builtin("RunPlugin(%s)" % nextep_info['url'])
