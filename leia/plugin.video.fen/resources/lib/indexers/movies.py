...
class Movies:
	...
	def build_movie_content(self, item_position, _id):
		try:
			...
			watched_params = {'mode': 'mark_movie_as_watched_unwatched', 'action': 'mark_as_watched', 'media_id': tmdb_id, 'title': title, 'year': item['year']}
			unwatched_params = {'mode': 'mark_movie_as_watched_unwatched', 'action': 'mark_as_unwatched', 'media_id': tmdb_id, 'title': title, 'year': item['year']}
			trakt_manager_params = {'mode': 'trakt_manager_choice', 'tmdb_id': item['tmdb_id'], 'imdb_id': item['imdb_id'], 'tvdb_id': item['tvdb_id'], 'db_type': 'movie'}
			...
			if resumetime != '0':
				cm.append((self.clearprog_str, 'RunPlugin(%s)' % build_url({"mode": "watched_unwatched_erase_bookmark", "db_type": "movie", "media_id": tmdb_id, "refresh": "true"})))
