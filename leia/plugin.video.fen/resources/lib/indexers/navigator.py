...
from modules.nav_utils import setView, translate_path, sleep, execute_builtin
...
class Navigator:
	...
	def downloads(self):
		...
		self._add_dir({'mode': 'browser_image', 'folder_path': images_path, 'foldername': 'Images File Downloads', 'list_name': list_name_insert % images_string}, name_insert % images_string, iconImage='people.png', isFolder=False)
		self._end_directory()
	...
	def premium(self):
		...
		if furk: self._add_dir({'mode': 'navigator.furk', 'foldername': 'Furk', 'list_name': furk_string}, name_insert % furk_string, iconImage='furk.png')
		if easynews: self._add_dir({'mode': 'navigator.easynews', 'foldername': 'Easynews', 'list_name': easy_string}, name_insert % easy_string, iconImage='easynews.png')
	...
	def set_view_modes(self):
		set_views_string, lists_string, root_string, movies_string, tvshows_string, season_string, episode_string, premium_files_string = ls(32510), ls(32501), ls(32457), ls(32028), ls(32029), ls(32537), ls(32506), ls(32485)
	...
	def changelogs(self):
		fen_string, changelog_string, fenomscrapers_string, myaccounts_string, log_viewer_string, kodi_string = ls(32036), ls(32508), ls(32522), ls(33025), ls(32509), ls(32538)
		...
		main_text_file, main_heading = translate_path(os.path.join(addon_dir, "resources", "text", "changelog.txt")), '[B]%s : [/B] %s  [I](v.%s)[/I]' % (changelog_string.upper(), fen_string, fen_version)
		scrapers_text_file, scrapers_heading = translate_path(os.path.join(translate_path(settings.ext_addon('script.module.fenomscrapers').getAddonInfo('path')), "changelog.txt")), '[B]%s : [/B] %s  [I](v.%s)[/I]' % (changelog_string.upper(), fenomscrapers_string, scrapers_version)
		myaccounts_text_file, myaccounts_heading = translate_path(os.path.join(translate_path(settings.ext_addon('script.module.myaccounts').getAddonInfo('path')), "changelog.txt")), '[B]%s : [/B] %s  [I](v.%s)[/I]' % (changelog_string.upper(), myaccounts_string, myaccounts_version)
		kodi_log_location, log_heading = os.path.join(translate_path('special://logpath/'), 'kodi.log'), '[B]%s : [/B]%s %s' % (log_viewer_string.upper(), kodi_string, log_viewer_string)
		self._add_dir({'mode': 'show_text', 'heading': main_heading, 'file': main_text_file, 'foldername': main_heading, 'exclude_external': 'true'}, main_heading, iconImage=fen_icon, isFolder=False)
		self._add_dir({'mode': 'show_text', 'heading': scrapers_heading, 'file': scrapers_text_file, 'foldername': scrapers_heading, 'exclude_external': 'true'}, scrapers_heading, iconImage=scrapers_icon, isFolder=False)
		self._add_dir({'mode': 'show_text', 'heading': myaccounts_heading, 'file': myaccounts_text_file, 'foldername': myaccounts_heading, 'exclude_external': 'true'}, myaccounts_heading, iconImage=myaccounts_icon, isFolder=False)
		self._add_dir({'mode': 'show_text', 'heading': log_heading, 'file': kodi_log_location, 'foldername': 'Kodi Log Viewer', 'exclude_external': 'true'}, log_heading, iconImage='lists.png', isFolder=False)
		self._end_directory()
	...
	def tips(self):
		tips_location = translate_path(os.path.join(addon_dir, "resources", "text", "tips"))
		...
		for tip in files:
			try:
				...
				action = {'mode': 'show_text', 'heading': tip_name, 'file': translate_path(os.path.join(tips_location, tip))}
	...
	def shortcut_folders(self):
		...
		delete_str = ls(32703)
		all_str = ls(32704)
		icon = os.path.join(self.icon_directory, 'genre_foreign.png')
		for i in folders:
			try:
				cm = []
				name = i[0]
	...
	def adjust_main_lists(self, params=None):
		...
		try:
			if method == 'display_edit_menu':
				...
				execute_builtin("Container.Update(%s)" % self._build_url({'mode': mode, 'action': action, 'url_mode': url_mode, 'menu_type': menu_type, 'query': query, 'db_type': db_type}))
	...
	def adjust_shortcut_folder_lists(self, params=None):
		...
		try:
			if method == 'display_edit_menu':
				...
				dbcon.commit()
				window.clearProperty('fen_%s_shortcut_folder' % list_name)
	...
	def _changelog_info(self):
		...
		changelog_file, changelog_heading = translate_path(os.path.join(addon_dir, "resources", "text", "changelog.txt")), '[B]Fen Changelog[/B]  [I](v.%s)[/I]' % addon_version
		show_text(changelog_heading, file=changelog_file)
