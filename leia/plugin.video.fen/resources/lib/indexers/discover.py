...
import json
from modules.nav_utils import build_url, setView, show_text, notification, translate_path, sleep, execute_builtin
...
class Discover:
	...
	def similar_recommended(self):
		...
		for item in results:
			...
			icon = 'https://image.tmdb.org/t/p/w92%s' % item['poster_path'] if item.get('poster_path') else translate_path(settings.addon().getAddonInfo('icon'))
	...
	def network(self):
		...
		for item in networks:
			...
			listitem.setArt({'icon': item['logo']})
			listitem.setProperty('id', str(item['id']))
			listitem.setProperty('name', name)
	...
	def export(self):
		try:
			...
			execute_builtin('RunPlugin(%s)' % self._build_url(url_params))
		except:
	...
	def help(self):
		text_file = translate_path(os.path.join(addon_dir, "resources", "text", "discover_help.txt"))
		return show_text(self.heading_base % ls(32487), file=text_file)
