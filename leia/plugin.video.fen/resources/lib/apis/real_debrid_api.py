...
class RealDebridAPI:
	...
	def refreshToken(self):
		window.setProperty('fen.refreshing.rd.token', 'true')
		...
		self.token = get_setting('rd.token')
		self.refresh = get_setting('rd.refresh')
		window.clearProperty('fen.refreshing.rd.token')
	...
	def _get(self, url):
		while window.getProperty('fen.refreshing.rd.token') == 'true':
			sleep(500)
		original_url = url
	...
	def clear_cache(self):
		try:
			...
			RD_DATABASE = translate_path('special://profile/addon_data/plugin.video.fen/fen_cache2.db')
