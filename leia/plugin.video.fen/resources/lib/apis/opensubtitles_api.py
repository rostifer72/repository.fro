...
try: from StringIO import StringIO as zipTool
except ImportError: from io import BytesIO as zipTool
try: from urllib import quote
except ImportError: from urllib.parse import quote
import json
from caches import fen_cache
from modules.nav_utils import notification, sleep
...
class OpenSubtitlesAPI:
	...
	def search(self, query, imdb_id, language, season=None, episode=None):
		...
		if cache:
			...
			_cache.set(cache_name, response, expiration=timedelta(hours=24))
		return response

	def download(self, url):
		response = self._get(url, stream=True, retry=True)
		content = gzip.GzipFile(fileobj=zipTool(response.content)).read()
		if py_tools.isPY3: content = py_tools.ensure_text(content, errors='ignore')
		return content
