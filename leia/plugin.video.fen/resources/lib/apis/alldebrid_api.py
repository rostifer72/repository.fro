...
class AllDebridAPI:
	...
	def auth(self):
		...
		progressDialog.update(-1, line % (ls(32517), ls(32700) % response.get('base_url'), ls(32701) % response.get('pin')))
	...
	def clear_cache(self):
		try:
			...
			AD_DATABASE = translate_path('special://profile/addon_data/plugin.video.fen/fen_cache2.db')
