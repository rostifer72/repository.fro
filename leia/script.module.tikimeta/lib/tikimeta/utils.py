...
def migrate_settings():
    if __addon__.getSetting('migration_complete') == 'true': return
    import xbmc
    import shutil
    try:
        old_folder = xbmc.translatePath('special://userdata/addon_data/script.module.furkitmeta')
        new_folder = xbmc.translatePath('special://userdata/addon_data/script.module.tikimeta')
        shutil.copytree(old_folder, new_folder)
        __addon__.setSetting('migration_complete', 'true')
        notification('Furkitmeta settings and database migrated to tikimeta', time=10000)
    except: pass
