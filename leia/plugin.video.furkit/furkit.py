...
if not mode or 'navigator.' in mode:
    ...
    if 'trakt.' in mode:
        ...
        if mode == 'trakt_sync_watched_to_furkit':
            from ast import literal_eval
            from resources.lib.modules.trakt import sync_watched_trakt_to_furkit
            sync_watched_trakt_to_furkit(literal_eval(params['refresh']))
    ...
    if mode in ('play_media'):
        ...
        from resources.lib.modules.sources import Sources
        Sources().display_results(literal_eval(params.get('use_dialog')), literal_eval(params.get('from_library')))
    elif mode == 'play_file':
        from resources.lib.modules.sources import Sources
    ...
    if mode == 'scraper_color_choice':
        ...
        from resources.lib.modules.nav_utils import unaired_episode_color_choice
