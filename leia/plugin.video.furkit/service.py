...
class AutoUpdater:             
    def runProgram(self):
        xbmc.log("[Furk It] Subscription service starting...")
                    ...
                    if now > next_run:
                        if xbmc.Player().isPlaying() == False:
                            if xbmc.getCondVisibility('Library.IsScanningVideo') == False:      
                                xbmc.log("[Furk It'] Updating video subscriptions")
                                time.sleep(1)
                                xbmc.executebuiltin('RunPlugin(plugin://plugin.video.furkit/?&mode=update_subscriptions)')
                                time.sleep(1)
                                __addon__.setSetting('service_time', str(datetime.datetime.now() + timedelta(hours=hours)).split('.')[0])
                                xbmc.log("[Furk It] Subscriptions updated. Next run at " + __addon__.getSetting('service_time'),2)
                        else:
                            xbmc.log("[Furk It] Player is running, waiting until finished")
                except: pass
            xbmc.sleep(5000)
