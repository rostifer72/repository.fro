...
from resources.lib.modules.trakt import sync_watched_trakt_to_furkit, get_trakt_movie_id
...
class Movies:
    ...
    def fetch_list(self):
        try:
            ...
            if self.total_pages: add_dir({'mode': 'build_navigate_to_page', 'db_type': 'Movies', 'current_page': page_no, 'total_pages': self.total_pages, 'transfer_mode': mode, 'transfer_action': self.action, 'passed_list': passed_list, 'foldername': self.action, 'query': search_name, 'actor_id': params.get('actor_id', '')}, 'Jump To...', iconImage='item_jump.png')
        ...
        for i in sorted(self.items, key=lambda k: k['item_no']):
            try:
                ...
                if not not_widget:
                    ...
                    listitem.setProperty("furkit_playcount", str(i['playcount']))
