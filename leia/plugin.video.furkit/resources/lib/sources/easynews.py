...
class EasyNewsSource:
    ...
    def results(self, info):
        ...
        for item in files:
            try:
                ...
                self.video_quality = self._get_release_quality(self.file_name, item['full_item'])
    ...
    def _get_release_quality(self, release_name, item=None):
        vid_quality = None
        if 'width' in item:
            if item['width'] == '0': vid_quality = None
            if item['width'] == '1920':
                vid_quality = '1080p'
            elif item['width'] == '1080':
                vid_quality = '720p'
            elif int(item['width']) > 1920:
                vid_quality = '4K'
            elif 0 < int(item['width']) < 720:
                vid_quality = 'SD'
        if not vid_quality:
