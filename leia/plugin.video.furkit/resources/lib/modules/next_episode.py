...
PROGRESS_NEXT_EPISODE_VIEW = 'view.progress_next_episode'
...
def build_next_episode():
    ...
    try:
        ...
        notification('Error getting Trakt Info', time=3500)
        pass

def process_eps(item, ne_settings, ne_display):
    from datetime import datetime
    import time
    meta = tikimeta.tvshow_meta('tmdb_id', item['tmdb_id'])
    ...
    try:
        ...
        result.append(build_episode({"tmdb_id": item['tmdb_id'], "season": season, "episode": episode,
...
def add_next_episode_unwatched(action=None, media_id=None, silent=False):
    ...
    if action == 'add': command, line1 = "INSERT OR IGNORE INTO unwatched_next_episode VALUES (?)", '%s Added to Furk It Next Episode' % params.get('title')
    else: command, line1 = "DELETE FROM unwatched_next_episode WHERE media_id=?", '%s Removed from Furk It Next Episode' % params.get('title')
    dbcon = database.connect(WATCHED_DB)
...
def build_next_episode_manager():
    ...
    if action == 'manage_unwatched':
        ...
        heading = 'Select Show to remove from Furk It Next Episode:'
        ...
        heading = 'Select Show to Include/Exclude in Furk It Next Episode:'
    ...
    if not tmdb_list:
        ...
        for tmdb_id in tmdb_list:
            try:
                meta = tikimeta.tvshow_meta('tmdb_id', tmdb_id)
                if action == 'manage_unwatched':
...
def next_episode_color_choice(setting=None):
    ...
    __addon__.setSetting(setting, chosen_color)
...
def next_episode_options_choice(setting=None):
    ...
    choices = [('Sort Type', 'nextep.sort_type', [('RECENTLY WATCHED', '0'), ('AIRDATE', '1'), ('TITLE', '2')]),
            ...
            ('Include Trakt or Furk It Unwatched', 'nextep.include_unwatched', [('OFF', 'false'), ('ON', 'true')]),
...
def next_episode_context_choice():
    ...
    choices = [
            ...
            ('INCLUDE WATCHLIST/UNWATCHED: [I]Currently [B]%s[/B][/I]' % unwatched_status, 'toggle_unwatched'),
            ...
            ('UNWATCHED HIGHLIGHT: [I]Currently [B]%s[/B][/I]' % unwatched_highlight, 'Unaired'),
    ...
    string = 'Please Choose Next Episode Setting to Change'
    ...
    if choice in ('toggle_sort_order', 'toggle_unaired', 'toggle_unwatched', 'toggle_airdate'):
...
def next_episode_from_playback(meta, from_library=None):
    ...
    window.setProperty('furkit_previous_meta_from_nextep', json.dumps(meta))
    info = {}
    if settings.autoplay_next_prompt(from_library):
        ...
        if playcount == max_playcount:
            continue_playing = xbmcgui.Dialog().yesno('Furk It Next Episode', '[B]Are you still watching %s?[/B]' % meta['title'], '', '', 'Not Watching', 'Still Watching', 10000)
...
def build_next_episode_play(meta, season, episode, from_library=None):
    ...
    infoLabels = season_episodes_meta(meta['tmdb_id'], season)
