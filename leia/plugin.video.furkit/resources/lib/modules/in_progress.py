...
def in_progress_tvshow(db_type=None):
    from tikimeta import tvshow_meta
    items = []
...
def build_in_progress_episode():
    ...
    def process_eps(item):
        listitem = build_episode({"tmdb_id": item[0], "season": int(item[1]), "episode": int(item[2]), "meta": tvshow_meta('tmdb_id', item[0])})['listitem']
