...
def download(name, image, url):
    ...
    if not media_folder:
        resp = xbmcgui.Dialog().yesno(
            ...
            "Furk It requires you to set a Download Folder.",
...
def done(title, dest, downloaded):
    ...
    text = xbmcgui.Window(10000).getProperty('FURKIT-DOWNLOADED')
    ...
    xbmcgui.Window(10000).setProperty('FURKIT-DOWNLOADED', text)

    if (not downloaded) or (not playing): 
        xbmcgui.Dialog().ok(title, text)
        xbmcgui.Window(10000).clearProperty('FURKIT-DOWNLOADED')

def doDownload(url, dest, title, image, headers):
    _addon__ = xbmcaddon.Addon(id='plugin.video.furkit')
    ...
    if xbmcgui.Dialog().yesno('Furk It' + ' - Confirm Download', file, 'Complete file is %dMB' % mb, 'Continue with download?', 'Confirm',  'Cancel') == 1:
        return
