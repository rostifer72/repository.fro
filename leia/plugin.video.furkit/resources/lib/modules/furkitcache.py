...
ADDON_ID = "plugin.video.furkit"


class FurkItCache(object):
    ...
    def check_cleanup(self):
        ...
        lastexecuted = self._win.getProperty("furkitcache.clean.lastexecuted")
        if not lastexecuted:
            self._win.setProperty("furkitcache.clean.lastexecuted", repr(cur_time))
    ...
    def _get_db_cache(self, endpoint, checksum, cur_time):
        ...
        result = None
        query = "SELECT expires, data, checksum FROM furkitcache WHERE id = ?"
    ...
    def _set_db_cache(self, endpoint, checksum, expires, data):
        ...
        query = "INSERT OR REPLACE INTO furkitcache( id, expires, data, checksum) VALUES (?, ?, ?, ?)"
        data = repr(data)
    ...
    def _do_cleanup(self):
        ...
        if self._win.getProperty("furkitcachecleanbusy"):
            return
        self._win.setProperty("furkitcachecleanbusy", "busy")

        query = "SELECT id, expires FROM furkitcache"
        for cache_data in self._execute_sql(query).fetchall():
            ...
            if cache_data[1] < cur_timestamp:
                query = 'DELETE FROM furkitcache WHERE id = ?'
        ...
        self._win.setProperty("furkitcache.clean.lastexecuted", repr(cur_time))
        self._win.clearProperty("furkitcachecleanbusy")
    ...
    def _get_database(self):
        ...
        try:
            ...
            try:
                connection = sqlite3.connect(dbfile, timeout=30, isolation_level=None)
                connection.execute(
                    """CREATE TABLE IF NOT EXISTS furkitcache(
    ...
    def _execute_sql(self, query, data=None):
        ...
        result = None
        error = None
        # always use new db object because we need to be sure that data is available for other FurkItcache instances
    ...
    def _log_msg(msg, loglevel=xbmc.LOGDEBUG):
        ...
        xbmc.log("FurkItcache --> %s" % msg, level=loglevel)
...
def use_cache(cache_days=14):
    '''
        wrapper around our furkit cache to use as decorator
        Usage: define an instance of FurkItcache with name "cache" (self.cache) in your class
