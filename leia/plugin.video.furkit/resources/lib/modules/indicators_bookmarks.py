...
FURKIT_DB = os.path.join(profile_dir, "furkit_cache.db")
        ...
        if watched_indicators in (1, 2):
            ...
            if not passed_list:
                ...
                for item in watched_list:
                    watched = get_watched_status_tvshow(item[0], aired_episode_number_tvshow(tikimeta.tvshow_meta('tmdb_id', item[0])))
...
def mark_episode_as_watched_unwatched():
    ...
    if watched_indicators in (1, 2):
        ...
        trakt_watched_unwatched(action, 'episode', imdb_id, season, episode)
...
def mark_season_as_watched_unwatched():
    ...
    if watched_indicators in (1, 2):
        from resources.lib.modules.trakt import trakt_watched_unwatched
        trakt_watched_unwatched(action, 'season', imdb_id, season)
        clear_trakt_watched_data('tvshow', imdb_id)
    if watched_indicators in (0, 1):
...
def mark_tv_show_as_watched_unwatched():
    ...
    if watched_indicators in (1, 2):
        from resources.lib.modules.trakt import trakt_watched_unwatched
        trakt_watched_unwatched(action, 'shows', imdb_id)
        clear_trakt_watched_data('tvshow', imdb_id)
    if watched_indicators in (0, 1):
        title = params.get('title', '')
        year = params.get('year', '')
        ...
        se_list = []
        count = 1
        s_data = tikimeta.tvshow_meta('tmdb_id', media_id)
...
def mark_movie_as_watched_unwatched():
    params = dict(parse_qsl(sys.argv[2].replace('?','')))
...
def mark_as_watched_unwatched(db_type='', media_id='', action='', season='', episode='', title=''):
    ...
    if action == 'mark_as_watched':
        dbcon.execute("INSERT OR IGNORE INTO watched_status VALUES (?, ?, ?, ?, ?, ?)", (db_type, media_id, season, episode, last_played, title))
...
def clear_trakt_watched_data(db_type, media_id=None):
    settings.check_database(FURKIT_DB)
    action = 'trakt_indicators_movies' if db_type in ('movie', 'movies') else 'trakt_indicators_tv'
    dbcon = database.connect(FURKIT_DB)
    dbcur = dbcon.cursor()
    dbcur.execute("DELETE FROM furkitcache WHERE id=?", (action,))
    dbcon.commit()
    window.clearProperty(action)
    if db_type == 'tvshow': clear_trakt_next_episode_data(media_id)
    dbcur.execute("DELETE FROM furkitcache WHERE id=?", ('trakt_watched_shows',))
    dbcon.commit()
...
def clear_trakt_next_episode_data(media_id=None):
    ...
    try:
        dbcon = database.connect(FURKIT_DB)
        dbcur = dbcon.cursor()
        dbcur.execute("SELECT * FROM furkitcache WHERE id = ?", ('trakt_watched_shows',))
    ...
    if media_id:
        try:
            ...
            dbcur.execute("DELETE FROM furkitcache WHERE id=?", ('trakt_view_history_%s' % str(trakt_id),))
            dbcon.commit()
    ...
    if not media_id:
        ...
        dbcur.execute("DELETE FROM furkitcache WHERE id LIKE 'trakt_view_history%'")
        dbcon.commit()
