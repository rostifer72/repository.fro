...
class Favourites:
    ...
    def add_to_favourites(self):
        try:
            ...
            dbcon.execute("INSERT INTO favourites VALUES (?, ?, ?)", (self.db_type, str(self.tmdb_id), self.title))
            dbcon.commit()
            dbcon.close()
    ...
    def get_favourites(self, db_type):
        ...
        else: result = [{'tmdb_id': str(i[0]), 'title': str(i[1])} for i in result]
        return result
    ...
    def add_to_favourites_audio(self):
        try:
            self.db_type = 'audio'
            self.title = self.dialog.input('Choose Name for New Furk It Music Favourite', type=xbmcgui.INPUT_ALPHANUM, defaultt=self.name)
            self.tmdb_id = '%s<>%s<>%s<>%s' % (self.title, self.id, self.url_dl, self.size)
