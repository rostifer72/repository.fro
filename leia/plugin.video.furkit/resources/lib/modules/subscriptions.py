...
class Subscriptions:
    ...
    def remake_subscriptions(self):
        ...
        for count, item in enumerate(subscriptions, 1):
            display = 'Furk It Updating %s' % self.choice[0]
    ...
    def update_subscriptions(self):
        ...
        for count, item in enumerate(subscriptions, 1):
            ...
            display = 'Furk It Updating - [B][I]%s[/I][/B]' % self.rootname
            bg_dialog.update(int(float(count) / float(len(subscriptions)) * 100), 'Please Wait', '%s' % display)
            xbmc.sleep(300)
        __addon__.setSetting('service_time', str(datetime.now() + timedelta(hours=hours)).split('.')[0])
        bg_dialog.close()
        notification('{}'.format('Furk It Subscriptions Updated'), 4500)
    ...
    def add_list_to_subscriptions(self, user, list_slug):
        if not xbmcgui.Dialog().yesno('Are you sure?','Furk It will add all Movies and/or TV Shows from this list to your Furk It Subscriptions. Do you wish to continue?'):
            return
    ...
    def add_remove_database(self):
        dbcon = database.connect(self.sub_database, timeout=40.0)
        dbcur = dbcon.cursor()
        dbcur.execute(self.db_action, (self.db_type, self.tmdb_id, self.title))
        dbcon.commit()
        dbcon.close()
