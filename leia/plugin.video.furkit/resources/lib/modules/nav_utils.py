...
def build_navigate_to_page():
    ...
    chosen_start = xbmcgui.Dialog().select('Furk It', choice_list, useDetails=True)
    xbmc.sleep(500)
...
def paginate_list(item_list, page, letter, limit=20):
    if letter != 'None':
        ...
        start_index = next(i for i,v in enumerate(title_list) if v.startswith(beginswith_tuple))
...
def setView(view_type, delay=250):
    try:
...
def remove_unwanted_info_keys(dict_item):
    remove = ('fanart_added', 'art', 'cast', 'imdb_id', 'item_no', 'poster', 'rootname', 'tmdb_id', 'tvdb_id',
        'fanart', 'all_trailers', 'total_episodes', 'total_seasons', 'total_watched', 'total_unwatched',
        'poster', 'fanart', 'banner', 'clearlogo', 'clearart', 'landscape')
...
def clear_cache(cache):
    ...
    if cache == 'meta':
        ...
        LIST_DATABASE = os.path.join(profile_dir, 'furkit_cache.db')
        if not xbmcvfs.exists(LIST_DATABASE): return
        if not xbmcgui.Dialog().yesno('Are you sure?','Furk It will Clear all List Data.'): return
        try: from sqlite3 import dbapi2 as database
        except ImportError: from pysqlite2 import dbapi2 as database
        dbcon = database.connect(LIST_DATABASE)
        dbcur = dbcon.cursor()
        dbcur.execute("DELETE FROM furkitcache")
